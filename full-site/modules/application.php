...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if(empty($name) OR empty($email) OR $count<2)
    {
		 ...
		 echo json_encode(array('result'=>0,'message'=>'Заполены не все поля'));
       }	
	   else  {
...
$result = $db->prepare("INSERT INTO `calls` (`dates`, `ip`, `name`, `fhonenumber`, `texts`, `typs`, `email`,`kv`,`crm`) VALUES (?,?,?,?,?,?,?,?,?)");
$result->bind_param('sssssissi', $today, $ip, $name, $phone, $formData, $typs, $email,$kv,$crm);
$result->execute();
//$result->close();
		return $db->insert_id; 
}
