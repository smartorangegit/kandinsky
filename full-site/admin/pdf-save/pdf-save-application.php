...
//*** Генерування Pdf
		...
		$result = $db->prepare("SELECT floor, 	sec, 	number, id, buld FROM `apartments` WHERE visible=1 ORDER BY id LIMIT $step OFFSET $offsetDb");
	
		$result->execute();
		$result->store_result();	$num=$result->num_rows;


		$result->bind_result($b['floor'],$b['sec'],$b['number'],$b['id'],$b['buld']);
		
		while($result->fetch()){
		$b['flat']=$b['number']{0}.'_'.$b['id'];
